...
public class test : MonoBehaviour
{
    ...
    void Start()
    {
        페이드_아웃("RoadView", 1, 30);
    }

    /// <summary>
    /// 오브젝트를 지정된 시간동안 기본 투명도까지 점점 선명하게 만들어 줍니다.
    /// 1 초 마다 "초당 조정 횟수" 만큼 투명도를 변화시켜 부드럽게 표현할 수 있습니다.
    /// 초당 조정 횟수는 30 이상을 추천합니다.
    /// </summary>
    public void 페이드_인(string 오브젝트_이름, float 걸리는_시간_초단위, float 초당_조정_횟수)
    {
        ...
        // N 초 동안 자연스럽게 a 값을 높일 코루틴을 구현합니다.     
        IEnumerator UpAlpha(float 걸리는_시간_초단위)
        {
            // 조정할 총 a 값 견적
        ...
        // 구현한 코루틴을 시작하여, a 값을 0 부터 원래 값 까지 높입니다.
        StartCoroutine(UpAlpha(걸리는_시간_초단위));
    }

    /// <summary>
    /// 오브젝트를 지정된 시간동안 완전 투명해질 때 까지 점점 투명하게 만들어 줍니다.
    /// 1 초 마다 "초당 조정 횟수" 만큼 투명도를 변화시켜 부드럽게 표현할 수 있습니다.
    /// 초당 조정 횟수는 30 이상을 추천합니다.
    /// </summary>
    public void 페이드_아웃(string 오브젝트_이름, float 걸리는_시간_초단위, float 초당_조정_횟수)
    {
        // 적용 대상 게임 오브젝트를 지정합니다.
        GameObject gameObject = GameObject.Find(오브젝트_이름);
        ...
        // N 초 동안 자연스럽게 a 값을 줄일 코루틴을 구현합니다.     
        IEnumerator UpAlpha(float 걸리는_시간_초단위)
        {
            // 조정할 총 a 값 견적
            float alphaRange = originalColor.a;
