...
public class 텍스트_이펙트
{
    ...
    public IEnumerator 페이드_인(string 오브젝트_이름, float 걸리는_시간_초단위, float 초당_조정_횟수)
    {
        ...
        // 일단 원래 컬러를 저장해둡니다.
        Color originalColor = gameObject.GetComponent<Text>().color;

        // 오브젝트를 투명하게 만듭니다. 즉, color 의 a (alpha, 투명도) 값을 0 으로 (0 = 최소값, 1 = 최대값) 바꿔야 합니다.
        // Text 컴포넌트의 color 구조체는 Constant 이므로, 새로 만들어서 바꿔줘야 합니다.
        gameObject.GetComponent<Text>().color = new Color(originalColor.r, originalColor.g, originalColor.b, 0);
